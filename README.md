...
<div>
  ...
  <p>Если бы только был какой-то способ научить Redux обращаться с функциями как с действиями…

  Ну, это именно то, что делает redux-thunk: это промежуточное программное обеспечение, которое отслеживает каждое действие, проходящее через систему, и, если это функция, она вызывает эту функцию. Это все, что он делает.</p>

  <p>Redux будет передавать два аргумента функциям-преобразователям: диспетчеризация, чтобы они могли отправлять новые действия, если им это нужно; и getState, чтобы они могли получить доступ к текущему состоянию. Таким образом, вы можете делать такие вещи:</p>
